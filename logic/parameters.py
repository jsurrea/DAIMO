...
def load_parameters(dfs):
    """
    Load parameters from the dictionary of dataframes
    ...
    df_puentes["nodo_inicial"] = df_puentes["START_Y"].replace(",", ".") + "/" + df_puentes["START_X"].replace(",", ".")
    df_puentes["nodo_final"] = df_puentes["END_Y"].replace(",", ".") + "/" + df_puentes["END_X"].replace(",", ".")
    ...
    df_principal = df_principal[["arco", "C-2", "C-3-4", "C-5", ">C-5"]]
    df_od = df_od[["nodo_origen", "nodo_destino", "vehiculo", "demanda"]]
    df_puentes = df_puentes[["id_puente", "arco"]]
